...
if __name__ == '__main__':
    ...
    # 新增：支持生成数据的参数
    parser.add_argument('--generated', action='store_true', help='Use generated data (pkl format) instead of original mat data')
    parser.add_argument('--mouse-key', type=str, default='all_neuron_20210916', 
                        help='Mouse key for generated data (e.g., all_neuron_20210916)')
    ...
    if args.generated:
        # 加载生成的数据（pkl格式）
        print(f'\nLoading generated data from {args.scn_data_path}')
        print(f'Mouse key: {args.mouse_key}')
        available_mice = datautils.get_available_mice(args.scn_data_path)
        print(f'Available mice: {available_mice}')
        
        if args.mouse_key not in available_mice:
            raise ValueError(f"Mouse key '{args.mouse_key}' not found. Available: {available_mice}")
        
        train_data, poi = datautils.load_generated_SCN(args.scn_data_path, args.mouse_key, args.task)
    else:
        # 加载原始数据（mat格式）
        train_data, poi = datautils.load_SCN(args.scn_data_path, args.task)
    
    print(f'Data loaded. Shape: {train_data.shape}, POI shape: {poi.shape}')
    ...
    # 修改run_dir命名，区分生成数据和原始数据
    if args.generated:
        run_dir = 'training/' + args.task + '_generated_' + args.mouse_key + '_' + name_with_datetime(args.run_name)
    else:
