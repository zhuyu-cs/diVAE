"""
batch_train.py
Batch training for generated SCN data
...
def export_ply_with_label(out, points, colors):
    """Export point cloud file with labels"""
...
def train_single_scn(train_data, poi, output_dir, config, device):
    """
    Train a single SCN and save results.

    Args:
        train_data: training data (N, num_trials, trial_length)
        poi: position information (N, 3)
    ...
    # Unsupervised clustering (1-5 classes)
    for num_classes in range(1, 6):
        ...
        # Compute class means for sorting
        ...
        plt.savefig(f'{output_dir}/tsne_{num_classes}.png', dpi=200)  # Also save png for easy viewing
        plt.close()
