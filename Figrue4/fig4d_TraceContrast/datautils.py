...
def load_generated_SCN(pkl_path, mouse_key, task='standard'):
    """
    Load generated SCN data (pkl format).

    Args:
        pkl_path: path to the pkl file, e.g. 'generated_activity_repeat1.pkl'
        mouse_key: mouse identifier, e.g. 'SCN1'
        task: task type
...
def get_available_mice(pkl_path):
    """
    Get all available mouse identifiers from a pkl file.

    Args:
        pkl_path: path to the pkl file

    Returns:
        list: list of mouse identifiers
    """
    with open(pkl_path, 'rb') as f:
        all_mice_data = pickle.load(f)
