...
class diVAE(nn.Module):
    ...
    def forward(self, cal_signal, time_label, mice): #video, behavior, pupil_centers, ca, mice):
        # ca: B N t
        # mice: id of mouse.
        # behavior = torch.cat([behavior, pupil_center], dim=-1)
        # project
        ...
        # preserved latent dynamics and idiosyncratic latent dynamics
        plds_ca, codebook_loss = self.pld(embed_ca, time_label)   # B c N t
        ilds_ca, kl_loss = self.ilds(embed_ca, mice)           # B c N t
        # time_smooth = torch.sum((plds_ca[..., 1:] - plds_ca[..., :-1])**2)+torch.sum((ilds_ca[..., 1:] - ilds_ca[..., :-1])**2)
        # decode
        restore_proj_ca = self.decoder(torch.cat([ilds_ca, plds_ca], dim=1)) # B c N t
        # restore ca to ouput
        restored_ca = self.output_projectors(restore_proj_ca, mice)
        return restored_ca, codebook_loss, kl_loss


    def inference(self, cal_signal, time_label, mice): 
        # ca: B N t
        # mice: id of mouse.
        # project
        proj_ca = self.in_projectors(cal_signal) # compress original trace to the same spatial size (Neuron), eg., B 1 7440 32 -> B 128 1024 32 
